...
export function getImprovementTips(score: number): Array<{ icon: string; text: string }> {
  ...
  if (score < 20) {
    return [tips[0], tips[2], tips[3]];
  } else if (score < 45) {
    return [tips[0], tips[1], tips[4]];
  } else if (score < 70) {
    return [tips[1], tips[4], tips[5]];
  }
  return [tips[2], tips[4], tips[5]];
}
...
export function getWhyDescription(score: number): string {
  ...
  return "Siz platformamyzyň iň abraýly agzasy! Başgalara görelde bolýarsyňyz we ýokary ynamdan peýdalanýarsyňyz.";
}
...
export function formatRelativeTime(isoString: string): string {
  ...
  return new Date(isoString).toLocaleDateString("tk-TM");
}
