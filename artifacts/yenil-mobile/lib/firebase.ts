...
export async function deductBalance(deviceId: string, amount: number): Promise<boolean> {
  try {
    let succeeded = false;
    await runTransaction(ref(db, `user-balances/${deviceId}`), (current) => {
      if (current === null) return undefined;
      const real  = current.balance      ?? 0;
      const bonus = current.bonusBalance ?? 0;
      if (real + bonus < amount) return undefined;
      const newBonus = Math.max(0, bonus - amount);
      const remainder = amount - (bonus - newBonus);
      const newReal   = Math.max(0, real - remainder);
      succeeded = true;
      return { ...current, balance: newReal, bonusBalance: newBonus, updatedAt: new Date().toISOString() };
    });
    return succeeded;
  } catch {
    return false;
